...
def help():
    print('Usage:')
    print('    RegWorkshopKeygen.py <name> [license_count]')
    print('')
    print('<name>:')
    print('    The name licensed to.')
    print('    This parameter must be specified.')
    print('')
    print('[license_count]:')
    print('    A integer from 1 to 500.')
    print('    This parameter is optional and default value is 1.')
    print('')
    print('e.g.')
    print('    ./RegWorkshopKeygen.py foobar 10')
    print('')


if __name__ == '__main__':
    if len(sys.argv) == 2:
        print(GenLicenseCode(sys.argv[1], 1))
    elif len(sys.argv) == 3:
        license_count = None

        try:
            license_count = int(sys.argv[2])
        except:
            help()
            exit(0)

        print(GenLicenseCode(sys.argv[1], license_count))

    else:
        help()
else:
    print('Please run this python script directly.')
